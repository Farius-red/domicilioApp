...
export class RegisterPage implements OnInit {
  login: register = {
    email: '', password: '', nombres: '', apellidos: '', confirmPassword: '', telefono: '', fechaNacimiento: '', imagen: '',
    genero: '',
  };
  submitted = false;
  ...
  onLogin(form: NgForm) {
    ...
    if (form.valid) {
      ...
      if (format.test(this.login.nombres)) {
        this.util.errorToast(this.util.translate('Please enter name'));
        return false;
      }
      if (format.test(this.login.apellidos)) {
        this.util.errorToast(this.util.translate('Please enter lastname'));
        return false;
      }
      ...
      if (format.test(this.login.telefono)) {
        this.util.errorToast(this.util.translate('Please enter cellphone'));
        return false;
      }

      this.isLogin = true;
      this.api.register(this.login.email, this.login.password, this.login.nombres, this.login.apellidos, this.login.telefono, this.login.fechaNacimiento, this.login.imagen, this.fechaRegistro, this.login.genero).then((userData) => {
