...
export class EditProfilePage implements OnInit {
  ...
  telefono: any = '';
  idUrl:any=Math.random().toString(36).substr(2, 8);
  saldo: number = 0;
  numerosrifa:any[]=[];
  ...
  async getProfile() {
    try {
      this.util.show();
      let data = await this.api.getProfile(localStorage.getItem('uid')).then((data: any) => data);
      this.util.hide();
      if (data) {
  ...
  update() {
    if ([ this.nombres, this.apellidos, this.telefono].some(e => !e || e == '')) {
    ...
    if (format.test(this.nombres) || format.test(this.apellidos)) {
